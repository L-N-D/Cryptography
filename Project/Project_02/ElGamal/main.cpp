...
vector<BigInt> readFile (string fileName){
    ...
    return {BigInt::hexToBig(p), BigInt::hexToBig(g), BigInt::hexToBig(x), BigInt::hexToBig(c1), BigInt::hexToBig(c2)};
...
// Find h = g^x (mod p)
BigInt mulMod (BigInt x, BigInt k, BigInt N){ //y = x^k (mod N)
...
BigInt nonNegativeGcdExtended(BigInt e, BigInt delta)
{
...
BigInt findInvMod (BigInt s, BigInt p){

    return nonNegativeGcdExtended(s, p);

}

BigInt decrypt (vector<BigInt> c, vector<BigInt> d){
    ...
    BigInt s = mulMod(c1, x, p);

    BigInt invS = findInvMod(s, p);

    BigInt m = (c2 * invS).mod(p);
    return m;
...
int main (int argc, char* argv[]) {
    ...
    vector<BigInt> d = {p, g, x};
    vector<BigInt> c = {c1, c2};

    BigInt h = mulMod(g, x, p);
    BigInt m = decrypt(c, d);
