...
vector<BigInt> readFile (string fileName){
    ...
    return {BigInt::hexToBig(p), BigInt::hexToBig(g), BigInt::hexToBig(a), BigInt::hexToBig(b)};
...
BigInt calPublicKey (BigInt x, BigInt k, BigInt N){
...
int main (int argc, char* argv[]) {
    ...
    BigInt aliceKey = calPublicKey(g, a, p);
    BigInt bobKey = calPublicKey(g, b, p);
    BigInt commonKey = calPublicKey(g, a*b, p);

    fo << aliceKey.bigToHex() << endl;
    fo << bobKey.bigToHex() << endl;
    fo << commonKey.bigToHex();
